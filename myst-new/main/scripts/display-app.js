...
function listenToQueue() {
    ...
    const qScores = query(collection(db, 'scores'), where('stageId', '==', currentMatId));
    unsubScores = onSnapshot(qScores, snap => {
        scoresMap = {};
...
// Math Engine Helper
function calculateFinalScore(trackNo, qItem) {
    if (!qItem || !qItem.j1_status || !qItem.j2_status || !qItem.j3_status || !qItem.j4_status || !qItem.j5_status) return "...";
    
    const s = scoresMap[trackNo] || {};
    const panelSize = s.panelSize || qItem.panelSize || 5;

    const getJ = (prefix) => {
        if (s[`${prefix}_a1`] === undefined) return null;
        let total = 0;
    ...
    const panel = [getJ('j1'), getJ('j2'), getJ('j3'), getJ('j4'), getJ('j5')].filter(v => v !== null);
    
    let olympicTotal = 0;
    if (panelSize === 3) {
        olympicTotal = panel.reduce((a, b) => a + b, 0);
    } else {
        olympicTotal = panel.reduce((a, b) => a + b, 0) - (panel.length > 0 ? Math.max(...panel) : 0) - (panel.length > 0 ? Math.min(...panel) : 0);
    }
...
function generateLiveRow(qItem) {
    ...
    const formatCell = (jObj, isLocked, isGhost) => {
        if (isGhost) return `<span class="text-gray-900">-</span>`; // Hide ghosts
        if (isLocked && jObj.total !== null) return `<span class="text-white">${jObj.total.toFixed(1)}</span>`;
    ...
    return `
        <tr class="hover:bg-gray-800/50 transition-colors border-b border-gray-800/50 animate-fade-in h-[20%]">
            ...
            <td class="py-0.5 md:py-1 px-1 md:px-2 text-center">${formatCell(j3, qItem.j3_status, false)}</td>
            <td class="py-0.5 md:py-1 px-1 md:px-2 text-center">${formatCell(j4, qItem.j4_status, panelSize === 3)}</td>
            <td class="py-0.5 md:py-1 px-1 md:px-2 text-center">${formatCell(j5, qItem.j5_status, panelSize === 3)}</td>
