...
function syncMatCard(clone, docId, data, busyStaff) {
    ...
    // ==========================================
    const header = clone.querySelector('.bg-slate-950\\/80');
    ...
    if (data.status === 'ready') {
        ...
        allSelects.forEach(select => {
            ...
            select.classList.add('opacity-40', 'cursor-not-allowed', 'bg-slate-950/80');
        });

        // 3. Disable the Lock Button so it can't be clicked again
        lockBtn.disabled = true;
        lockBtn.innerText = "PANEL SECURED";
        lockBtn.classList.replace('from-emerald-600', 'from-slate-800');
        lockBtn.classList.replace('to-teal-600', 'to-slate-800');
        lockBtn.classList.replace('hover:from-emerald-500', 'hover:from-slate-800');
        lockBtn.classList.replace('hover:to-teal-500', 'hover:to-slate-800');
        lockBtn.classList.replace('text-white', 'text-slate-500');
        lockBtn.classList.replace('shadow-lg', 'shadow-none');
        lockBtn.classList.add('cursor-not-allowed', 'border', 'border-slate-700');
    } else {
        ...
        allSelects.forEach(select => {
            ...
            select.classList.remove('opacity-40', 'cursor-not-allowed', 'bg-slate-950/80');
        });

        lockBtn.disabled = false;
        lockBtn.innerHTML = '<span class="mr-2">🔒</span> Lock Staff Panel';
        lockBtn.classList.replace('from-slate-800', 'from-emerald-600');
        lockBtn.classList.replace('to-slate-800', 'to-teal-600');
        lockBtn.classList.replace('hover:from-slate-800', 'hover:from-emerald-500');
        lockBtn.classList.replace('hover:to-slate-800', 'hover:to-teal-500');
        lockBtn.classList.replace('text-slate-500', 'text-white');
        lockBtn.classList.replace('shadow-none', 'shadow-lg');
        lockBtn.classList.remove('cursor-not-allowed', 'border', 'border-slate-700');
    }
}
